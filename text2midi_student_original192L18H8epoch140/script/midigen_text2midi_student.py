...
def build_parser():
    ...
    # ✅ NEW: resume KD from an Accelerate checkpoint directory (recommended),
    # or from a single .bin file (weights-only).
    s.add_argument(
        "--resume_path",
        type=str,
        default=None,
        help="Resume KD from checkpoint dir (e.g., .../epoch_136 or .../step_12345) "
            "or from a weights file (e.g., .../epoch_136/pytorch_model.bin).",
    )
...
def distill_accelerate(
    ...
    resume_path: Optional[str] = None,   # ✅ NEW
):
    per_device_train_batch_size = configs['training']['text2midi_model']['per_device_train_batch_size']
    gradient_accumulation_steps = configs['training']['text2midi_model']['gradient_accumulation_steps']
    lr_scheduler_type = configs['training']['text2midi_model']['lr_scheduler_type']
    ...
    # =============================================================================
    # ✅ NEW: Resume KD training (Accelerate state OR weights-only)
    # =============================================================================
    start_epoch = 0
    start_step = 0

    if resume_path:
        # allow user to pass either a directory (best) or a file
        resume_dir = resume_path
        resume_file = None
        ...
        # 1) Best-case: resume full accelerate state if directory exists and looks like a checkpoint
        if os.path.isdir(resume_dir):
            try:
                accelerator.load_state(resume_dir)
                accelerator.print(f"[RESUME] Loaded full Accelerate state from: {resume_dir}")

                # derive start_epoch from folder name if possible (epoch_136 -> start at epoch index 135)
                base = os.path.basename(os.path.normpath(resume_dir))
                m = re.match(r"epoch_(\d+)$", base)
                if m:
                    start_epoch = max(int(m.group(1)) - 1, 0)
                    start_step = start_epoch * num_update_steps_per_epoch
                else:
                    # step_12345 checkpoints
                    m2 = re.match(r"step_(\d+)$", base)
                    if m2:
                        start_step = max(int(m2.group(1)), 0)
                        start_epoch = start_step // max(1, num_update_steps_per_epoch)

            except Exception as e:
                accelerator.print(f"[RESUME] accelerator.load_state failed ({e}); falling back to weights-only.")

        # 2) Fallback: weights-only load if user passed a .bin (or if load_state failed)
        if resume_file and os.path.exists(resume_file):
            try:
                ...
                accelerator.unwrap_model(student).load_state_dict(state_dict, strict=False)
                accelerator.print(f"[RESUME] Loaded student weights from: {resume_file}")
            except Exception as e:
                raise RuntimeError(f"Failed to load resume weights from {resume_file}: {e}")
